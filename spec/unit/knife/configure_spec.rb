...
describe Chef::Knife::Configure do
  ...
  it "writes the new data to a config file" do
    ...
    @knife.run
    expect(config_file.string).to include("client_name     = '#{Etc.getlogin}'")
    expect(config_file.string).to include("client_key      = '/home/you/.chef/#{Etc.getlogin}.pem'")
    expect(config_file.string).to include("chef_server_url = '#{default_server_url}'")
  end
