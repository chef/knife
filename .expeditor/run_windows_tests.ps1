...
$gem_source1 = "$env:ARTIFACTORY_ENDPOINT/omnibus-gems-local/gems/chef-19.1.36-universal-mingw-ucrt.gem"
...
gem install chef-utils   --version ">=19.1" --source $gem_source
gem install chef-config  --version ">=19.1" --source $gem_source
...
Write-Host "--- Downloading chef gem with correct platform name"
$gem_url = "$gem_source"
$downloaded_path = "$env:TEMP"
